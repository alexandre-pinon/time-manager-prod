defmodule TimeManager.ClockManagerTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2021-10-25 10:57:00]}

      assert {:ok, %Clock{} = clock} = ClockManager.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2021-10-25 10:57:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2021-10-26 10:57:00]}

      assert {:ok, %Clock{} = clock} = ClockManager.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2021-10-26 10:57:00]
    end
